...
public:
  ...
  static void initParameters(REAL kappa, REAL dielp, REAL diels, int nmol, REAL rs);
  ...
  CProtein(const char * fname);
	...
	/*! Function that determines whether two molecules have collided or not
			...
			\return a boolean indicating whether the molecules have collided (true) */
  ...
  //void QCDoutput(ostream & fout); 
	...
	/*! Function that computes forces and torques on a group of CProtein objects
...
//													CPnt & trans) const;
  ...
  static REAL computeDistance(const CProtein & p1, const CProtein & p2)
	...
	/*! CProtein translate function
			\param trans a vector of XYZ coordinates to translate the CProtein object by */		
  ...
  void setOrient(const CQuat & rot)
	...
	/*! CProtein untransform function, move protein center back to saved location */	
  void untransform()
  ...
  REAL getCharge(int i) const
	...
	/*! CProtein getChargePos function
				\param i the ith atom in the protein for which to obtain charge position
  ...
  static map<int, REAL> CHARGES[NUM_AAS];	//!< Array of charges for each amino acid in the protein.
private:
...
// Variables
  static bool m_initialized;				//!< Operator that indicates whether the protein constants are initialized
  static vector<CMPE*> m_exps;					//!< A vector of expansions that are kept for each protein in the system
  static vector<CPnt*> m_cens;					//!< A vector of positions for each protein in the system
  static vector<CProtein*> m_mols;			//!< A vector of protein classes, one for each protein in the system.			
  static bool m_bFirst;									//!< Operator, designates whether a force calc has been performed  (true=no)
  
  vector<CAtom> m_atoms;								//!< A vector of all the atoms in the protein
  vector<const CAtom*> m_chargedAtoms;	//!< A vector of all the charged atoms in the protein
  ...
  CPnt m_center;												//!< A cartesian coordinate object of current position
	CPnt m_undoCenter;										//!< A saved molecule coordinate, used for undoing a translation
