...
//! Hard link to the file that contains a list of all OPLS charges for each AA
 ...
 ******************************************************************/
void CProtein::initParameters(REAL kappa, REAL dielp, REAL diels, int nmol,
		REAL rs) {
...
void CProtein::loadChargeMap() {
	...
	while (!fin.eof()) {
		...
		if (r != 3) {
			cout << "Bad input line in file " << PARAM_FILE << " : " << buf
					<< endl;
			exit(0);
		}
 ...
 ******************************************************************/
CPnt CProtein::computeCenter() {
	CPnt p;

	for (int i = 0; i < m_atoms.size(); i++) p += m_atoms[i].getPos();
 ...
 ******************************************************************/
bool CProtein::inCollision(const CProtein &mol) {
	if (CProtein::computeDistance(*this, mol) > getRadius() + mol.getRadius())
		return false;
	else
		return true;
}
 ...
 * Determine which of all of the atoms in the CG simulation has the
             largest radius.
 ******************************************************************/
REAL CProtein::getMaxAtomRad() {
	REAL max = 0.0;
	for (int i = 0; i < m_atoms.size(); i++)
		if (m_atoms[i].getRadius() > max) max = m_atoms[i].getRadius();
 ...
 *
 ******************************************************************/
/*void CProtein::PDBoutput(ostream & fout, int sindex, const char * chainid,
 ...
 *
 ******************************************************************/
/*void CProtein::QCDoutput(ostream & fout) {
	char buf[100];
 ...
 ******************************************************************/
const CAtom *CProtein::getAtom(int resnum, int acode) const {
	for (int i = 0; i < m_atoms.size(); i++)
 ...
 ******************************************************************/
void CProtein::getInterfaceAtoms(const CProtein &P1, const CProtein &P2,
		vector<CPnt> &pos) {
	...
	for (; it1 != A1.end(); it1++) {
		if (!it1->isHeavy()) continue;

		it2 = A2.begin();
		for (; it2 != A2.end(); it2++) {
			if (!it2->isHeavy()) continue;
 ...
 *        system
 ******************************************************************/
void CProtein::computeForces(vector<CPnt> &force, vector<CPnt> &torque) {
	vector<REAL> pot;
	...
	// Update the solution to mutual polarization.
