...
/******************************************************************/
/******************************************************************//**
*  Reset rotation WRT an axis and an angle
******************************************************************/
void
	...
	for (int m = 0; m < m_p-1; m++)
		for (int n = m+2; n < 2*m_p-m-1; n++)
			for (int s = -n+1; s < n; s++)
			{
				...
				ROT(n-1,s,m+1) = m_exxi*(dum1*m_exiphi+dum2*m_exphi+dum3)/ETA(n,m); // EQ 1.1
			}
}
...
CRotCoeff::computeGradCoeff()
{
  ...
  for (int n = 1; n < 2*m_p-1; n++)				// EQ 1.3
	{
      ...
      for (int s = -n+1; s < n; s++)
			{
				...
				dROT(n-1,s,m+1) = m_exxi*(dum1*m_exiphi+dum2*m_exphi+dum3)/ETA(n,m); // EQ 1.4
			}
}
...
{
  REAL SH[2*N_POLES-1];
  CSHCoeff::specialSH(SH, 2*N_POLES-1, 1.0);  // = -1*sqrt((n-1)!/(n+1)!)*P(n,1(1.0))
