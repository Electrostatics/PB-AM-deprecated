...
CBD::CBD(const char *fname1, const char *fname2, REAL kappa) : m_fcnt(0) {
	if (!CProtein::isInitialized())
		...
		// paper=30
		CProtein::initParameters(kappa, DIELECTRIC_PROT,
				DIELECTRIC_WATER, 2, 30);
...
void approxBBall(const vector<CPnt> &V, CPnt &cen, REAL &rad) {
	...
	REAL dx2 = dVx.normsq(), dy2 = dVy.normsq(),
			dz2 = dVz.normsq(); // diffs squared
...
REAL buildSystem(const char *ifname, int num, REAL dist, bool bSave,
	...
	for (int i = 0; i < p1.size(); i++) // moving the positions of each charge
	                                    // to be WRT the center of the mol
		p1[i] -= cn;
	...
	CProtein::initParameters(KAPPA, DIELECTRIC_PROT, DIELECTRIC_WATER, num,
	                         rad1);
	for (int n = 0; n < num; n++) {
		// Chose random orientation if 3+ mol. in system
		...
		if (bSave) { // If we wish to save configurations
		             ...
		             // And one containing the CG sphere
...
int run_slv(const char * ifname, int num, double dist) {
	...
	buildSystem(ifname, num, dist, true, mpe, cen);
...
int run_dif(const char * ifname, int num, REAL dist) {
	...
	vector<CMPE *> mpe;
	vector<CPnt *> cen;
	// Build a system of num molecules
	REAL rad = buildSystem(ifname, num, dist, true, mpe, cen);
...
int run_rad(const char *ifname, REAL fact) {
	...
	// print out a PQR file without the CG sphere
